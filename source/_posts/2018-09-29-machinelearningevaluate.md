...
- True Positive Rate (TPR)：代表将正例分对的概率
    - $$ TPR = \frac{TP}{TP + FN} $$

- False Positive Rate (FPR)：代表将负例错分为正例的概率
    - $$ FPR = \frac{FP}{FP + TN} $$
...
- 受试者曲线 ROC
    ...
    - 在ROC空间中，每个点的横坐标是 **FPR(假正数-负样本被预测为正样本的个数/概率)**，纵坐标是 **TPR(真正数-正样本被预测为正样本的个数/概率)**，这也就描绘了分类器在 TP(真正的正例) 和 FP(错误的正例) 间的 trade-off。
